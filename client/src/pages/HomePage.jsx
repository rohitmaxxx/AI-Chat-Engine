...
const HomePage = () => {
  ...
  const getAnswer = async () => {
    ...
    console.log("========response: ", response, response.sourceDocuments)
    if (response) {
      setMessages([...newMessages, {
        type: messageType.answer,
        content: response.text,
        references: response.sourceDocuments
      }]);
      setOnRequest(false);
    }
        ...
        <Box sx={{
          ...
          {messages.map((item, index) => (
            <Box key={index} padding={1}>
              ...
              <Box sx={{
                padding: 2,
                bgcolor: item.type === messageType.answer && "#808080",
                borderRadius: 3,
                width: "100%"
              }}>
                <div>
                    <ul>
                        {
                          item.references.map((val, key) => {
                            return <li><h3>Page.{key + 1}</h3>{val.pageContent}
                            <br />
                            {/* <span>Meta source: </span><a href={val.metadata.source}>{val.metadata.source}</a> */}
                            <ul>
                                <li>
                                  <span>Pdf source: </span><a href={val.metadata.source}>{val.metadata.source}</a>
                                </li>
                                <li>
                                  <span>Pdf pages: </span>{val.metadata.pdf_numpages}
                                </li>
                            </ul>
                            </li>
                          })
